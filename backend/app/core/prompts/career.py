def build_career_prompt(
    ...
    system_prompt = """════════════════════════════════════════════════════════════════
  SYSTEM PROMPT — "Career Reading"  v1
...
Korean output:
  ...
  CRITICAL:
    GOOD: "토(土)의 기운이 깔려 있어요."
    BAD:  "Wood(木) 에너지", "Gap(甲) 일주"  ← 절대 금지
...
Korean 출력 시: 괄호 안 영어 병기 절대 금지.
  금지: `봉사(Acts of Service)`, `안정형(Secure)`,
        `염소자리(Capricorn)` 등
  허용: `봉사`, `안정형`, `염소자리`
...
🌍 2. 에너지가 살아나는 업무 세계 / 🌍 2. The Aligned Environment
...
🤝 3. 성장시키는 인간관계의 흐름 / 🤝 3. Human Dynamics

협업 스타일, 리더십 성향, 성장을 이끄는 관계 패턴.

Draw from: Moon sign + career direction (풀어서 설명, MC 표기 금지)
1–2 paragraphs.


💰 4. 돈과 기회가 따라오는 흐름 / 💰 4. Wealth Blueprint
...
🔋 5. 오래 빛나기 위한 회복 메커니즘 / 🔋 5. Energy Protection
...
🌟 6. 재능이 가장 크게 확장되는 분야 / 🌟 6. Destiny Domain

타고난 재능이 가장 빛나는 분야. 장기적 커리어 방향.

Draw from: Career direction (풀어서 설명) + Sun sign's
           highest expression + chart strength
...
════════════════════════════════════════════════════════════════
  ...
  Structure:  Opening + 섹션 1–6 + Final Message (번호 없음)
